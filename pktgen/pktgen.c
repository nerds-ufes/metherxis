...
/* timestamp array size */
#define MAX_TSARRAY 4096
#define tstamp_norm(x) (ntohs(x) % MAX_TSARRAY)
...
static u64 tv_nsec[MAX_TSARRAY];	/* the timestamp array global */
					...
					struct pktgen_dev *pkt_dev)
{
	...
	*(__be16 *) & eth[12] = protocol;
	...
	// timestamp
	tv_nsec[tstamp_norm(iph->id)] = ktime_to_ns(ktime_get());
...
static int 
latency_calc(unsigned int idx, ktime_t now, struct pktgen_rx *data_cpu)
{
	u64 latency = 0;
	u64 jitter = 0;
	ktime_t ktime_tx;
	if (!is_pktgen_sending)
		return 0;
	//ktime_tx.tv64 = ((u64)ntohl(pgh->tv_sec) << 32) | ntohl(pgh->tv_usec);
	ktime_tx.tv64 = tv_nsec[idx];
...
unsigned int pktgen_rcv_counter(unsigned int hooknum, struct sk_buff *skb, const struct net_device *in, const struct net_device *out, int (*okfn)(struct sk_buff *))
{
	...
	data_cpu = &get_cpu_var(pktgen_rx_data);
...
unsigned int
pktgen_rcv_time(unsigned int hooknum, struct sk_buff *skb, const struct net_device *in, const struct net_device *out, int (*okfn)(struct sk_buff *))
{
        //struct pktgen_hdr *pgh;
        ...
        struct iphdr *iph;
        struct udphdr *udph;
        //struct sk_buff *skbc = skb_clone(skb, GPF_ATOMIC);
        int ret = NF_DROP;
        //int kk;
        ...
        //      printk("iph->id: %d ;", ntohs(iph->id));
        //      for (kk=0; kk<skb->len; kk++)
        //              printk("%c", &skb->data+kk );
        //}

        //pgh = (struct pktgen_hdr *)(((char *)(skb_transport_header(skb)))+4);

        //pgh = (struct pktgen_hdr *)((char *)(iph + 28));
        //udph = (struct udphdr *)(skb_transport_header(skb) + ip_hdrlen(skb));
        udph = (struct udphdr *)skb_transport_header(skb);

/*
        if (debug) {
                printk("udph : PKTGEN_MAGIC_NET: %08x ; ", PKTGEN_MAGIC_NET);
                for (kk=0; kk<sizeof(udph); kk++)
                        printk("%c", &udph+kk);
        }


        skb_pull(skbc, sizeof(udph));
        udph = (struct udphdr *)skb_transport_header(skbc);
        pgh = (struct pktgen_hdr *)((char *)udph);
        pgh = (struct pktgen_hdr *)(14 + sizeof(struct iphdr) + sizeof(struct udphdr));



        if (debug) {
                printk("\npgh : PKTGEN_MAGIC_NET: %08x ; ", PKTGEN_MAGIC_NET);
                for (kk=0; kk<sizeof(pgh); kk++)
                        printk("%c", &pgh+kk);
        }


        if (debug) {
                //printk("pgh->pgh_magic: %08x ;", ntohl(pgh->pgh_magic));
                //printk("pgh->seq_num: %08x ;", ntohl(pgh->seq_num));
                //printk("pgh->tv_sec: %08x ;", ntohl(pgh->tv_sec));
                //printk("pgh->tv_usec: %08x;", ntohl(pgh->tv_usec));

              printk("udph->source %d ;", ntohs(udph->source));
              printk("udph->dest %d ; ", ntohs(udph->dest));
              printk("udph->len %d\n", ntohs(udph->len));
        }
*/


        if (unlikely((ntohs(udph->source) != 9) || (ntohs(udph->dest) != 9))){
                ret = NF_ACCEPT;
                goto end;
        }
        ...
        latency_calc(tstamp_norm(iph->id), now, data_cpu);
